...
def test_only_renders_all_formats(output_format: str) -> None:
    ...
    result = render_metafile(plus_file([fill_rect()]), output_format=output_format, dpi=100)
...
def test_clear_replaces_existing_pixels_and_alpha() -> None:
    ...
    image = rgba(render_metafile(data, dpi=100).data)
    assert image.getpixel((20, 20))[3] == 0
    assert image.getpixel((70, 70)) == (0, 0, 255, 255)
    assert b"<image " in render_metafile(data, output_format="svg").data
...
def test_object_fragments_reassemble_before_use() -> None:
    ...
    assert rgba(render_metafile(data, dpi=100).data).getpixel((20, 20)) == (0, 255, 0, 255)
...
def test_unsupported_object_replaces_old_slot() -> None:
    ...
    result = render_metafile(data, dpi=100)
...
def test_unsupported_state_stops_drawing_but_checks_record_boundaries() -> None:
    ...
    result = render_metafile(plus_file(prefix), dpi=100)
    ...
    with pytest.raises(MetafileMalformedError):
        render_metafile(plus_file([*prefix, struct.pack("<HHII", 0x400A, 0, 1000, 988)]))
...
def test_getdc_window_and_dual_do_not_double_draw() -> None:
    ...
    image = rgba(render_metafile(dual, dpi=100).data)
    assert image.getpixel((20, 20))[3] == 0
    assert image.getpixel((70, 70))[:3] == (0, 255, 0)
...
def test_transform_order_and_state_restore() -> None:
    ...
    image = rgba(render_metafile(plus_file(records), dpi=100).data)
...
def test_page_units_preserve_physical_extent(unit: int, extent: float) -> None:
    ...
    doc = parse_metafile(plus_file([plus_record(0x4030, struct.pack("<f", 1), unit), fill_rect(rect=(0, 0, extent, extent))]))
...
def test_coordinate_encodings(flags: int, points: bytes) -> None:
    ...
    image = rgba(render_metafile(plus_file([record]), dpi=100).data)
...
def test_relative_fifteen_bit_and_rle_path() -> None:
    ...
    image = rgba(
        render_metafile(
            plus_file([plus_object(0, 3, data), plus_record(0x4014, struct.pack("<I", 0xFF00FF00), 0x8000)]), dpi=100
        ).data
    )
...
def test_unicode_lines_and_driver_positions() -> None:
    ...
    commands = [c for c in parse_metafile(data).commands if isinstance(c, DrawTextCommand)]
    ...
    result = render_metafile(plus_file([fill_rect(), font_object(), plus_record(0x4036, bytes(glyph), 0x8000)]))
...
def test_raw_bitmaps_preserve_color_and_alpha(pixel_format: int, raw: bytes) -> None:
    ...
    result = render_metafile(plus_file([plus_object(0, 5, obj), draw]), dpi=100)
...
def test_invalid_or_empty_only_is_rejected(records: list[bytes]) -> None:
    ...
    with pytest.raises((MetafileMalformedError, MetafileUnsupportedError)):
        render_metafile(plus_file(records))
...
def test_nested_record_budgets(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(MetafileResourceLimitError):
        render_metafile(plus_file([fill_rect()]))
    monkeypatch.setattr(playback, "MAX_RECORDS", 100)
    monkeypatch.setattr(playback, "MAX_STATE_DEPTH", 1)
    with pytest.raises(MetafileResourceLimitError):
        render_metafile(plus_file([plus_record(0x4025, struct.pack("<I", 0)), plus_record(0x4025, struct.pack("<I", 1))]))
    monkeypatch.setattr(playback, "MAX_METAFILE_BYTES", 8)
    with pytest.raises(MetafileResourceLimitError):
        render_metafile(plus_file([plus_object(0, 1, solid(0xFF000000))]))
...
def test_native_gdiplus_only_fixtures(scene: str) -> None:
    ...
    result = render_metafile((root / f"{scene}-only.emf").read_bytes(), dpi=96)
    image = rgba(result.data)
    ...
    dual = render_metafile((root / f"{scene}-dual.emf").read_bytes(), dpi=96)
...
def test_bad_plus_header_cannot_fall_back_to_plain_emf() -> None:
    ...
    with pytest.raises(MetafileMalformedError):
        render_metafile(data)
    with pytest.raises(MetafileMalformedError):
        render_metafile(build_emf([plus_comment([fill_rect(), plus_record(0x4002)])]))
...
def test_missing_eof_and_duplicate_header_are_malformed() -> None:
    ...
    for records in [
        ...
        with pytest.raises(MetafileMalformedError):
            render_metafile(build_emf([plus_comment(records)]))
...
def test_continued_object_mismatch_and_oversize_fail_closed() -> None:
    ...
    for end in [
        ...
        with pytest.raises(MetafileMalformedError):
            render_metafile(plus_file([start, end]))
...
def test_clip_path_combination_and_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(MetafileResourceLimitError):
        render_metafile(plus_file([*records[:2], plus_record(0x4033, flags=0x100), fill_rect()]))


def test_quality_approximations_are_reported_with_offsets() -> None:
    """SourceCopy 等已知近似必须保留记录类型与文件偏移。"""
    result = render_metafile(plus_file([plus_record(0x4023, flags=1), plus_record(0x4021, flags=7), fill_rect()]))
    assert result.partial
...
def test_negative_stride_and_invalid_bitmap_dimensions() -> None:
    ...
    with pytest.raises(MetafileResourceLimitError):
        render_metafile(plus_file([plus_object(0, 5, bad), fill_rect()]))
...
def test_driver_string_extra_matrix_and_realized_advance() -> None:
    ...
    result = render_metafile(plus_file([font_object(), plus_record(0x4036, data, 0x8000)]))
    assert result.partial
    doc = parse_metafile(plus_file([font_object(), plus_record(0x4036, data, 0x8000)]))
...
def test_rendering_origin_consumes_both_coordinates() -> None:
    """非零首坐标不得通过短路漏读第二个坐标。"""
    result = render_metafile(plus_file([plus_record(0x401D, struct.pack("<ii", 5, 6)), fill_rect()]))
    assert result.partial
